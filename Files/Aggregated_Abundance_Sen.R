...
top4 <- ggplot(subset(total_reshape, Var2 %in% c("Coverage.METABAT_VerySensitive.1", "Coverage.METABAT_VerySensitive.2",
      "Coverage.METABAT_VerySensitive.3", "Coverage.METABAT_VerySensitive.4")), 
      aes(x = Var1, y= value, group = as.factor(Var2))) 
top4 + geom_bar(stat = "identity", aes (fill = Var2), position = "dodge") +
  theme(axis.text.x = element_text(angle = 50, hjust = 1)
  ) + 
